...
window.addEventListener('load', () => {
    ...
    const highScoreY = scoreY + 25; // Position High Score below Score (Step 15)
    ...
    // Added Step 15: High Score Storage Key
    ...
    // Updated Step 15: Added High Score display
    function drawUI(ctx) {
        ctx.font = uiFont;
        ...
        // Draw High Score (Step 15)
        ctx.fillText(`HI: ${highScore}`, scoreX, highScoreY);

        // Draw Wave Number
        ctx.textAlign = 'center';
        ctx.fillText(`WAVE: ${currentWave}`, canvasWidth / 2, scoreY);
        ...
        // Draw Life Icons
        ctx.fillStyle = lifeIconColor; ctx.strokeStyle = lifeIconColor; ctx.lineWidth = 1;
        for (let i = 0; i < player.lives; i++) { /* ... life icon drawing ... */
            const iXO=livesX+10+(i*lifeIconSpacing); const tX=iXO,tY=scoreY-5; const lX=iXO-5,lY=tY+10; const rX=iXO+5,rY=tY+10;
    ...
    // Added Step 15: Load High Score function
    function loadHighScore() {
        try {
            const storedScore = localStorage.getItem(HIGH_SCORE_KEY);
            if (storedScore !== null) {
                highScore = parseInt(storedScore, 10) || 0; // Parse and default to 0 if invalid
            ...
            console.error("Could not access localStorage to load high score:", e);
            highScore = 0; // Default to 0 if localStorage fails
        }
    }

    // Added Step 15: Check and Save High Score function
    function checkAndSaveHighScore() {
        if (score > highScore) {
            highScore = score;
            console.log(`New high score! Saving ${highScore} to localStorage.`);
            try {
                localStorage.setItem(HIGH_SCORE_KEY, highScore.toString());
            } catch (e) {
                console.error("Could not access localStorage to save high score:", e);
            }
        }
    }

    function resetGame() { // Updated Step 15 (minor: ensure high score is loaded initially)
        console.log("Resetting game state...");
        // highScore is loaded once on page load, no need to reload here usually
        // loadHighScore(); // Usually not needed here, loaded on page load
        score = 0;
        player.lives = initialPlayerLives;
        player.x = canvasWidth / 2; player.y = canvasHeight - 50;
        playerProjectiles.length = 0; enemies.length = 0;
        currentWave = 1; startWave(1);
        ...
        if (currentState === GameState.PLAYING) {
            ...
            for (let i = playerProjectiles.length - 1; i >= 0; i--) {
                ...
                for (let j = enemies.length - 1; j >= 0; j--) {
                    ...
                    if (collision) { enemies.splice(j, 1); score += scorePerEnemy; projectileHit = true; playSound('enemyHit'); break; }
                }
            ...
            for (let i = enemies.length - 1; i >= 0; i--) {
                ...
                if (collision) {
                    ...
                    if (player.lives <= 0) {
                        ...
                        checkAndSaveHighScore(); // Check/Save high score on Game Over (Step 15)
            ...
            drawUI(ctx); // Draws Score, HI Score, Wave, Lives

        } else if (currentState === GameState.GAME_OVER) {
            // Draw Game Over Screen (Updated Step 15 to show High Score)
             ...
             ctx.fillText('GAME OVER', canvasWidth / 2, canvasHeight / 2 - 60); // Adjusted position slightly
             ctx.font = gameOverFontMedium; ctx.fillStyle = uiColor;
             ctx.fillText(`Final Score: ${score}`, canvasWidth / 2, canvasHeight / 2 + 0); // Adjusted position
             ctx.fillText(`High Score: ${highScore}`, canvasWidth / 2, canvasHeight / 2 + 35); // Added High Score display
             ctx.fillText('Press ENTER to Restart', canvasWidth / 2, canvasHeight / 2 + 80); // Adjusted position
        }
    ...
    async function startGame() {
        loadHighScore(); // Load high score before starting anything else (Step 15)
        ...
        // currentState is set by resetGame now
